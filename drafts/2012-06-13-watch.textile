...
I wrote a patch for @watch@ that adds an optional @--chg-exit/-g@ command line argument for this behavior, and I'm happy to say it was "merged upstream":merge and "is available as of procps v3.3.3":announcement.  The patch makes possible things like @watch -g "curl http://localhost:3000/" && ./run_integration_tests.sh@ to wait for your webapp to start before running any tests, or @watch -g "nc -w 1 <ip address> 22" && ssh <ip address> restart_apps.sh@ to wait until the "OpenSSH daemon":openssh starts before restarting your applications on a remote server.
